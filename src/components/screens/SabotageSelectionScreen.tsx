...
export const SabotageSelectionScreen: React.FC<SabotageSelectionScreenProps> = ({
    ...
    return (
        <div className="flex flex-col h-full max-w-4xl mx-auto p-4 pt-16 pb-8 space-y-6 safe-area-padding relative">
            {/* Skip Button */}
            <button 
                onClick={onSkip}
                className="absolute top-4 right-4 text-gray-400 hover:text-white text-sm font-bold px-3 py-1 rounded-full border border-gray-600 hover:bg-gray-800 transition-all z-10"
            >
                SKIP
            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 flex-1 overflow-y-auto min-h-0 pb-4">
            ...
            <div className="pt-4 border-t border-white/10 safe-area-bottom-padding">
                ...
                >
                    {isSubmitting ? 'UNLEASHING...' : 'UNLEASH CHAOS ⚡️'}
                </button>
            </div>
        </div>
    );
