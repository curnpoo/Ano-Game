...
export const BentoToolbar: React.FC<BentoToolbarProps> = ({
    ...
    const cardBase = "bg-black/40 backdrop-blur-xl rounded-2xl border border-white/10 shadow-lg p-1.5 flex flex-col gap-1.5 relative overflow-visible"; // reduced rounding/padding for compact fit
        ...
        <div className="w-full max-w-lg mx-auto pointer-events-auto grid grid-cols-6 gap-2 p-2 safe-area-bottom-padding select-none relative">
            ...
            <div className={`col-span-1 ${cardBase} items-center justify-between`}>
                ...
                >
                    ...
                    {/* Size Popup */}
                    {showSizes && (
                        <div className="absolute left-[110%] bottom-0 mb-[-10px] bg-black/80 backdrop-blur-xl border border-white/20 rounded-xl p-2 flex flex-col gap-2 shadow-2xl z-50 animate-fade-in w-12">
                            {SIZES.map((s) => (
                                <button
                                    key={s.label}
                                    onClick={(e) => {
                                    ...
                                    className={`w-full aspect-square rounded-lg flex items-center justify-center transition-all ${brushSize === s.size ? 'bg-white text-black' : 'text-white/60 hover:bg-white/10'}`}
                                >
                    ...
                    {effectiveColors.map((color) => {
                        ...
                        return (
                            <button
                                ...
                                className={`aspect-square rounded-full transition-all duration-200 ${isSelected ? 'scale-110 shadow-lg ring-2 ring-white z-10' : 'hover:scale-110 opacity-80 hover:opacity-100'}`}
                                style={{
                                    backgroundColor: color,
                                    border: color === '#FFFFFF' ? '1px solid #aaa' : '1px solid rgba(255,255,255,0.1)',
                                }}
                            />
                        );
                    })}
