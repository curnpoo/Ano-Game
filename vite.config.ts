...
export default defineConfig(() => {
  ...
  const normalizedBase = rawBasePath.endsWith('/') ? rawBasePath : `${rawBasePath}/`
  const sanitizedBase = normalizedBase.startsWith('/') ? normalizedBase : `/${normalizedBase}`

  const baseSegment = sanitizedBase === '/' ? '' : sanitizedBase.replace(/^\/|\/$/g, '')
  const resolvedOutDir = process.env.VITE_BUILD_OUTDIR ?? (baseSegment ? path.join('dist', baseSegment) : 'dist')
